...
// ROTA CLIENTES ( get lista - IMPORTANTE)
...
// ROTA DE CADASTRO DE CLIENTE (subrota/cadastrar) (IMPORTANTE)
router.post("/clientes/cadastrar", Auth, (req, res) => {
  // Criando as variaveis que irão amarzenar os dados vindo do formulario
  const nome = req.body.nome;
  const cpf = req.body.cpf;
  const endereco = req.body.endereco;

  //  Enviando os dados para o banco
  // O metodo create cadastra informação no BD
  Cliente.create({
    // Coluna // variável
    nome: nome,
    cpf: cpf,
    ...
    .catch((error) => {
      console.log("Ocorreu um erro ao cadastrar cliente." + error);
    });
});
...
router.get("/clientes/editar/:id", Auth,(req, res) => {
  const id = req.params.id;
  // Buscando cliente no BD
  Cliente.findByPk(id).then((cliente) => {
    res.render("clienteEditar", {
      // Passando os dados do cliente para página
      cliente: cliente,
    });
  });
});
...
router.post("/clientes/alterar", Auth, (req, res) => {
  // Coletando os dados do fumalário
  const nome = req.body.nome;
  const cpf = req.body.cpf;
  const endereco = req.body.endereco;
